...
function keep_alive() {
    if (HOST === location.host) {
        ...
        AJAX({
            'data': {
                'cmd': 'user/user_acc,resume_online,keep_online,key_detail,user/user_html,get_credits,get_user_info,get_user_menu',
                'map': 'user/user_html/get_credits:user_list'
            },
            'callback': function (data) {
                if (data['user/user_acc/keep_online'] === false) clearInterval(timer);
                else {
                    ...
                    if (null !== sign_in) {
                        sign_in.href = data['user/user_html/get_user_menu']['news']['url'];
                        sign_in.innerText = data['user/user_html/get_user_menu']['news']['text'];
                    }
                    var sign_up = document.getElementById('sign_up');
                    if (null !== sign_up) {
                        sign_up.href = data['user/user_html/get_user_menu']['panel']['url'];
                        sign_up.innerText = data['user/user_html/get_user_menu']['panel']['text'];
                    }
                }
                if (0 < data['user/user_html/get_credits'].length) {
                    var credits = document.getElementById('credits_list');
                    var Key, credits_list = '';
                    for (Key in data['user/user_html/get_user_info']) credits_list += '<span><a href="/user/user.php?id=' + Key + '" target="_blank"><img src="' + FILE + data['user/user_html/get_user_info'][Key]['user_head'] + '">' + data['user/user_html/get_user_info'][Key]['user_name'] + '</a></span>';
